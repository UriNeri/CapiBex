...
create_exception!(capibex, FastxParseError, pyo3::exceptions::PyException);

// Avoid some boilerplate with the error handling
macro_rules! py_try {
    ($call:expr) => {
        $call.map_err(|e| PyErr::new::<FastxParseError, _>(format!("{}", e)))?
    };
}
...
impl Record {
    ...
    /// -------
    /// Record
    ///     A Record object with normalized sequence.
    ///
    /// See also
    /// --------
    ...
    pub fn normalize(&self, iupac: bool) -> PyResult<Record> {
        let normalized_seq = if let Some(s) = normalize(self.seq.as_bytes(), iupac) {
